...
function Badge({ children, tone }: { children: React.ReactNode; tone: string }) {
    return (
        <span className={`inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-medium ${badgeStyles[tone] || "bg-zinc-100 text-zinc-700 border-zinc-200"}`}>
            {children}
        </span>
    )
}
...
export default function InsightsClient({ initialInsights }: { initialInsights: ChatInsightRecord[] }) {
    ...
    return (
        <div className="min-h-screen bg-linear-to-br from-white via-zinc-50 to-stone-100 text-zinc-900">
            <div className="sticky top-0 z-40 border-b border-zinc-200 bg-white/80 backdrop-blur-xl">
                <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
                    <div>
                        <p className="text-sm font-medium uppercase tracking-[0.3em] text-zinc-500">Admin Dashboard</p>
            ...
            <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
                <motion.section
                    ...
                    className="mb-8 rounded-3xl border border-zinc-200 bg-white p-6 shadow-lg shadow-zinc-200/60"
                >
                    <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
                        <div>
                            <p className="text-sm font-medium text-zinc-500">Private analytics for the business owner only</p>
                        ...
                        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
                            <input
                                ...
                                className="col-span-full rounded-2xl border border-zinc-300 bg-white px-4 py-3 text-sm outline-none transition focus:border-zinc-900 focus:ring-2 focus:ring-zinc-900/10"
                            />
                            <select
                                value={filters.issueCategory}
                                onChange={(event) => setFilters((current) => ({ ...current, issueCategory: event.target.value }))}
                                className="rounded-2xl border border-zinc-300 bg-white px-3 py-3 text-sm outline-none transition focus:border-zinc-900 focus:ring-2 focus:ring-zinc-900/10"
                            >
                ...
                {visibleInsights.length === 0 ? (
                    <div className="rounded-3xl border border-dashed border-zinc-300 bg-white px-6 py-16 text-center text-sm text-zinc-500 shadow-sm">
                    ...
                    <div className="grid gap-5 xl:grid-cols-2">
                        {visibleInsights.map((insight, index) => (
                            <motion.article
                                ...
                                className="group rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm shadow-zinc-200/40 transition duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-zinc-200/70"
                            >
                                <div className="flex flex-wrap items-start justify-between gap-3">
                                    <div>
                                        <p className="text-xs font-medium uppercase tracking-[0.24em] text-zinc-400">Issue Title</p>
                                        <h3 className="mt-2 text-xl font-semibold tracking-tight text-zinc-900">{insight.title}</h3>
                                    </div>
                                    <Badge tone={insight.priority}>{insight.priority}</Badge>
                                </div>

                                <p className="mt-4 text-sm leading-6 text-zinc-600">{insight.summary}</p>
                                ...
                                <div className="mt-6 flex flex-col gap-4 border-t border-zinc-100 pt-5 sm:flex-row sm:items-center sm:justify-between">
                                    <p className="text-sm text-zinc-500">Created {formatDate(insight.createdAt)}</p>
                                    <button
                                        type="button"
                                        onClick={() => setSelectedInsight(insight)}
                                        className="inline-flex items-center justify-center rounded-xl border border-zinc-300 px-4 py-2 text-sm font-medium text-zinc-800 transition hover:border-zinc-900 hover:bg-zinc-900 hover:text-white"
                                    >
                                        View Conversation
                                    </button>
                                </div>
                ...
                {selectedInsight && (
                    ...
                    >
                        <motion.div
                            ...
                            className="max-h-[90vh] w-full max-w-4xl overflow-hidden rounded-3xl border border-zinc-200 bg-white shadow-2xl"
                            onClick={(event) => event.stopPropagation()}
                        >
                            <div className="flex items-start justify-between gap-4 border-b border-zinc-200 px-6 py-5">
                                <div>
                                    <p className="text-xs font-medium uppercase tracking-[0.24em] text-zinc-400">Conversation</p>
                                    <h3 className="mt-1 text-2xl font-semibold tracking-tight text-zinc-900">{selectedInsight.title}</h3>
                                </div>
                                <button
                                    type="button"
                                    onClick={() => setSelectedInsight(null)}
                                    className="rounded-full border border-zinc-300 px-3 py-1.5 text-sm font-medium text-zinc-600 transition hover:bg-zinc-100"
                                >
                                    Close
                                </button>
                            </div>
                                ...
                                <div className="grid gap-6 lg:grid-cols-[1fr_0.9fr]">
                                        ...
                                        {getConversationTurns(selectedInsight).length === 0 ? (
                                            ...
                                            getConversationTurns(selectedInsight).map((turn, index) => (
                                                <div key={`${turn.role}-${index}`} className={`flex ${turn.role === "Customer" ? "justify-start" : "justify-end"}`}>
                                                    <div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-6 ${turn.role === "Customer" ? "bg-zinc-100 text-zinc-800" : "bg-zinc-900 text-white"}`}>
                                    ...
                                    <div className="space-y-4 rounded-3xl border border-zinc-200 bg-zinc-50 p-5">
                                        <div className="grid gap-3 text-sm text-zinc-700">
                                            <div>
                                                <span className="block text-xs font-medium uppercase tracking-[0.24em] text-zinc-400">Category</span>
                                                <span className="mt-1 inline-flex">
                                                    <Badge tone={selectedInsight.issueCategory}>{selectedInsight.issueCategory}</Badge>
                                                </span>
                                            </div>
                                            <div>
                                                <span className="block text-xs font-medium uppercase tracking-[0.24em] text-zinc-400">Sentiment</span>
                                                <span className="mt-1 inline-flex">
                                                    <Badge tone={selectedInsight.sentiment}>{selectedInsight.sentiment}</Badge>
                                                </span>
                                            </div>
                                            <div>
                                                <span className="block text-xs font-medium uppercase tracking-[0.24em] text-zinc-400">Priority</span>
                                                <span className="mt-1 inline-flex">
                                                    <Badge tone={selectedInsight.priority}>{selectedInsight.priority}</Badge>
                                                </span>
                                            </div>
                                            <div>
                                                <span className="block text-xs font-medium uppercase tracking-[0.24em] text-zinc-400">Resolved</span>
                                                <span className="mt-1 inline-flex">
                                            ...
                                            <div>
                                                <span className="block text-xs font-medium uppercase tracking-[0.24em] text-zinc-400">Created</span>
                                                <p className="mt-1 text-sm text-zinc-700">{formatDate(selectedInsight.createdAt)}</p>
                                            </div>
                                            <div>
                                                <span className="block text-xs font-medium uppercase tracking-[0.24em] text-zinc-400">Summary</span>
                                                <p className="mt-1 text-sm leading-6 text-zinc-600">{selectedInsight.summary}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
